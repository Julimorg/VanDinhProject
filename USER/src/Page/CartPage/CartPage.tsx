...
import { Card, Button, InputNumber, Empty, Divider, Space, Image, Tag, Popconfirm, message } from 'antd';
import { DeleteOutlined, ShoppingCartOutlined, ArrowLeftOutlined } from '@ant-design/icons';
import { useGetAllCarts } from '../ProductPage/Hook/useGetAllCarts';
import { useAuthStoreCookiesStorage } from '../../Middleware/useAuthStore';

const CartPage: React.FC = () => {
  const { id: userId } = useAuthStoreCookiesStorage();
  const { data: cartResponse, isLoading, isError } = useGetAllCarts(userId ?? '');
  ...
  const calculateTotalQuantity = () => {
    return localCart.reduce((total, item) => total + item.product.productQuantity, 0);
  };
  ...
  const handleQuantityChange = (cartItemId: string, value: number) => {
    ...
    message.info('Thay đổi số lượng (chỉ UI)');
  };

  const handleRemoveItem = (cartItemId: string) => {
    setLocalCart(prev => prev.filter(item => item.cartItemId !== cartItemId));
    message.info('Xóa sản phẩm (chỉ UI)');
  };

  if (isLoading) return <p className="p-8 text-center">Loading giỏ hàng...</p>;
